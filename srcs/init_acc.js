...
puppeteer.use(RecaptchaPlugin({ provider: { id: '2captcha', token: '89f71b2cf02b35c6b5c1f8deb9f9161b' }, visualFeedback: true, throwOnError: true}))
...
async function init_twitter_pptr(account, legit) {
	...
	page.on('pageerror', async (msg) => {
		if (msg.message.includes("HTTP-403 codes:[64]") == true) {
			console.log(`User: ${account.user} is suspended`)
			await axios({
				method: 'DELETE',
				url: `http://twitter.faldin.xyz/api/delete/account?user=${account.user}`,
				headers: {
	...
	if (account.cookies.length !== 0) {
		...
		try {
			...
			console.log(`error while logging without cookies ${account.user}`)
			await browser.close()
			return (1)
		}
	...
	try {
		...
		console.log(`error while changing tag ${account.user}`)
		await page.screenshot({ path: process.cwd() + `/debug_screenshot/${account.user}_TAG.jpg`})
		await browser.close()
		return (1)
	}
...
async function update_profile(account, copy) {
	...
	try {
		...
		console.log(e)
		console.log(`${account.user} NOT OK UPDATE PROFILE`)
		return (1)
	}
}
